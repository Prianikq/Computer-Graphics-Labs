...
void main(void)
{
    vec3 L = vec3(LightPos.x - FragVertex.x, LightPos.y - FragVertex.y, LightPos.z - FragVertex.z); // LIGHT IN WORLD SPACE
    float dist = length(L);
    L = normalize(L);
    ...
    {
        vec3 S = vec3(CameraPos.x - FragVertex.x, CameraPos.y - FragVertex.y, CameraPos.z - FragVertex.z); // -1000 - FragVertex.z ???
        vec3 R = vec3(reflect(-L, FragNormale)); // vec3 ???
